...
export default function rtlMemoryConsolidatorPolicy(pi) {
  if (process.env.RTL_AGENT_PI_MEMORY_CONSOLIDATOR_POLICY_REQUIRED !== "1") return;
  ...
  const auditPath = process.env.RTL_AGENT_PI_MEMORY_CONSOLIDATOR_READ_AUDIT;
  if (
    auditPath === undefined ||
    !path.isAbsolute(auditPath) ||
    normalizedRelativePath(workspaceRoot, auditPath) !== "context/read-audit.jsonl"
  ) {
    throw new Error("RTL_AGENT_PI_MEMORY_CONSOLIDATOR_READ_AUDIT must bind the audit file");
  }
  writeFileSync(auditPath, "", { encoding: "utf8", flag: "wx" });
  pi.on("tool_call", async (event) => {
    if (!["read", "write"].includes(event.toolName)) {
      return { block: true, reason: "Tool is outside the locked Memory Consolidator allowlist" };
    }
    const input =
    ...
    return allowed
      ? undefined
      : { block: true, reason: "Path is outside the locked Memory Consolidator workspace policy" };
  });
}
