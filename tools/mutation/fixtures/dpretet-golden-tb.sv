...
module tb;
  ...
  async_fifo #(.DSIZE(8),.ASIZE(3),.FALLTHROUGH("TRUE")) dut(.wclk(si_clk),.wrst_n(si_res_n),.winc(shift_in),.wdata(d_in),.wfull(full),.awfull(almost_full),.rclk(so_clk),.rrst_n(so_res_n),.rinc(shift_out),.rdata(d_out),.rempty(empty),.arempty(almost_empty));
  ...
  initial begin
    ...
    for(j=0;j<8;j=j+1) begin
      ...
      if(!full) $fatal(1,"FULL_FLAGS");
      for(i=0;i<8;i=i+1)get;
      repeat(5) @(negedge so_clk);
      if(!empty) $fatal(1,"EMPTY_FLAGS");
    end
    fork
    ...
    $display("DPRETET_GOLDEN_PASS reads=%0d",reads);$finish;
  end
