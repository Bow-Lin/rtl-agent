...
const dir = path.join(publication, "ufifo");
const manifestBytes = await readFile(path.join(dir, "manifest.json"));
assert.equal(
  sha(manifestBytes),
  suite.ips.find((ip: { id: string }) => ip.id === "ufifo").manifestDigest,
);
...
const run = path.join(
  ...
  "ufifo-rx-depth16-width8",
  "run_20260915-064348-272",
);
const snapshots = path.join(run, "evidence", "verification-assets");
const dutPath = "rtl/ufifo.v";
...
for (const [stage, attempt] of [
  ...
  for (const mutant of [null, ...m.mutants]) {
    ...
    if (!compile.error && compile.exitCode === 0) {
      ...
      verdict = simulationVerdict(sim, "UFIFO_GOLDEN_PASS");
    }
