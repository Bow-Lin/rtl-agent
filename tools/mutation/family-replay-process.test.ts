...
test("shared serial ownership requires the exact token and current process", async () => {
  const o = await options();
  const dir = path.join(o.repo, ".rtl-agent", "fifo-target-campaigns");
  await mkdir(dir, { recursive: true });
  const lock = path.join(dir, "active.lock");
  await writeFile(lock, JSON.stringify({ token: "owned", pid: process.pid }));
  await assert.rejects(recordedProcess({ ...o, fifoLockToken: "other" }), /NOT_IDLE/);
  assert.equal((await recordedProcess({ ...o, fifoLockToken: "owned" })).exitCode, 0);
  await writeFile(lock, JSON.stringify({ token: "owned", pid: -1 }));
  await assert.rejects(
    recordedProcess({ ...o, label: "wrong-pid", fifoLockToken: "owned" }),
    /NOT_IDLE/,
  );
});
...
test("losing the shared serial token stops an already running command", async () => {
  const o = await options();
  const dir = path.join(o.repo, ".rtl-agent", "fifo-target-campaigns");
  await mkdir(dir, { recursive: true });
  const lock = path.join(dir, "active.lock");
  await writeFile(lock, JSON.stringify({ token: "owned", pid: process.pid }));
  const running = recordedProcess({
    ...o,
    args: ["-e", "setTimeout(()=>{},8000)"],
    fifoLockToken: "owned",
  });
  const rejection = assert.rejects(running, /CONCURRENT_RTL_OR_PAUSE/);
  await new Promise((resolve) => setTimeout(resolve, 150));
  await writeFile(lock, JSON.stringify({ token: "changed", pid: process.pid }));
  await rejection;
  const evidence = JSON.parse(await readFile(path.join(o.cwd, "test.json"), "utf8"));
  assert.equal(evidence.cleanupConfirmed, true);
  assert.equal(evidence.error, "CONCURRENT_RTL_OR_PAUSE");
});
