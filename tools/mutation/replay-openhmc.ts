...
const dir = path.join(publication, "openhmc");
const manifestBytes = await readFile(path.join(dir, "manifest.json"));
assert.equal(
  sha(manifestBytes),
  suite.ips.find((ip: { id: string }) => ip.id === "openhmc").manifestDigest,
);
...
const run = path.join(
  ...
  "openhmc-depth8-width8",
  "run_20260915-184524-998",
);
const snapshots = path.join(run, "evidence", "verification-assets");
const dutPath = "rtl/building_blocks/fifos/async/openhmc_async_fifo.v";
...
for (const [stage, attempt] of [
  ...
  const snapshot = JSON.parse(await readFile(path.join(source, "manifest.json"), "utf8"));
  assert.equal(snapshot.attempt, attempt);
  const assets = new Map<string, Buffer>(rawAssets);
  for (const entry of snapshot.entries) {
    assert.match(entry.path, /^rtl\/(?:dut\/)?[a-z_]+\.(?:sv|v)$/);
    ...
    if (entry.path.startsWith("rtl/dut/") && entry.path !== "rtl/dut/top_wrapper.sv") {
      const rawPath = entry.path.replace("rtl/dut/", "rtl/building_blocks/fifos/async/");
  ...
  for (const mutant of [null, ...m.mutants]) {
    ...
    if (!compile.error && compile.exitCode === 0) {
      ...
      verdict = simulationVerdict(sim, "OPENHMC_GOLDEN_PASS");
    }
