...
// This runner is deliberately local to family preparation. Existing FIFO workers are untouched.
export async function recordedProcess(o: RecordedProcessOptions) {
  ...
  if (present(pause) || present(fifo)) throw new Error("FAMILY_PREPARATION_NOT_IDLE");
  ...
  try {
    ...
    const monitor = setInterval(() => {
      try {
        if (present(fifo) || present(pause)) cancel("CONCURRENT_RTL_OR_PAUSE");
      } catch {
