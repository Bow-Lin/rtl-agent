...
const dir = path.join(publication, "eth-fifo");
const manifestBytes = await readFile(path.join(dir, "manifest.json"));
assert.equal(
  sha(manifestBytes),
  suite.ips.find((ip: { id: string }) => ip.id === "eth-fifo").manifestDigest,
);
...
const run = path.join(
  ...
  "eth-fifo-depth8-width32",
  "run_20260914-184218-176",
);
const snapshots = path.join(run, "evidence", "verification-assets");
const dutPath = "rtl/verilog/eth_fifo.v";
...
for (const [file, digest] of Object.entries(m.sourceHashes)) {
  assert.match(file, /^rtl\/verilog\/[a-z_]+\.v$/);
...
for (const [stage, attempt] of [
  ...
  for (const entry of snapshot.entries) {
    ...
    if (entry.path.startsWith("rtl/dut/") && entry.path !== "rtl/dut/top_wrapper.sv") {
      const rawPath = entry.path.replace("rtl/dut/", "rtl/verilog/");
  ...
  for (const mutant of [null, ...m.mutants]) {
    ...
    if (!compile.error && compile.exitCode === 0) {
      ...
      verdict = simulationVerdict(sim, "ETH_FIFO_GOLDEN_PASS");
    }
