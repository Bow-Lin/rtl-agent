...
- A03 `decide`，输入为 task projection 与 pending review 组装出的 `DomainState | null`。
- A04 transaction manager 和 repositories。
- 一个显式 `ExecutionContextFactory`，在 transaction 外生成 `occurredAt`、event IDs、outbox IDs。

`ExecutionContextFactory` 的生产实现可以用 `Date`/`crypto.randomUUID`，domain 仍然纯；测试使用固定序列实现。对于已有 task，`occurredAt` 至少等于当前 `updatedAt`（例如 `max(clock, updatedAt)`），避免系统时钟回拨制造非法 DecisionContext。Phase A 每个 command 精确提供一个 event ID；以后增加多事件 handler 时同步扩展数量计划。
...
7. Load current task projection and pending review, then assemble DomainState
8. Check expectedStateVersion
9. Call domain.decide(current, command, context)
   9a. deterministic business rejection -> store failure response, COMMIT
   9b. domain integrity/context failure -> abort without writing idempotency
10. Persist next task using insert/CAS and insert/update the review projection
...
`REQUEST_REVIEW` 同 transaction 插入完整 review row（包括 request actor/time）；`RECORD_REVIEW_DECISION` 同 transaction 更新该 row 的 status、decision、decision actor/time。Task 的 `pending_review_id` 与 review row 必须一起改变，不能只写其中一边。
...
- domain 业务拒绝：映射到对应 A02 稳定 code。
- `STATE_INVARIANT_VIOLATION`、`TRANSITION_INVARIANT_VIOLATION`、`INVALID_EVENT_BATCH`、`INVALID_EVENT_SEQUENCE`、`INVALID_DECISION_CONTEXT`：视为内部 application/storage corruption 或编程错误，安全映射为 `INTERNAL_ERROR`，不把 reason 暴露，也不作为确定性业务拒绝写入新的 idempotency row。
...
- domain 业务拒绝不改变 task/event/outbox，但其 failure response 可幂等重放；domain integrity/context failure 不写新 idempotency response。
...
corepack pnpm --filter @rtl-agent/application --fail-if-no-match test
