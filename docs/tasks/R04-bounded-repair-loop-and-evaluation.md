...
4. 保存严格 `AgentTurnResult`、turn 后 manifest 与 `rtl-after/**` 副本；不持久化 raw
   ...
   例外：实际 `before_provider_request` payload 与最终 Assistant `message_end` 按 exchange
   顺序写入忽略的
   `_internal/runs/<run-id>/evidence/attempts/<attempt>/provider-transcript.json`；中断的最终
   exchange 显式保留 `response: null`。该 transcript 不进入公开 summary、生成 RTL 或
   observed-issues。
...
2026-07-20 以相同的固定 archive/cache/Provider 边界接入 ChipBench commit `74fe7d283225ae030ef59326a06111c9d372b48e`。三个 `Verilog Gen` split 提供 45 个 `BLANK_GENERATION` case；八个 `Verilog Debugging` split 提供 178 个 `PROMPTED_FUNCTIONAL_REPAIR` case。调试 case 的 buggy RTL 位于 prompt 内，没有独立 starter baseline；其 timing/assignment/arithmetic/state-machine 功能是否修复不能由 R03 compile pass 证明。2026-08-06 增加按 split 的 Kimi profile 与私有 Icarus/VVP reference/testbench 仿真，功能结论仍为非权威证据。Ref Model Gen、Tool_Box 和上游执行脚本仍不提取、不执行。锁元数据位于 `core-loop/fixtures/chipbench.lock.json`。
