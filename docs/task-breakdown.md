...
**状态**：`DONE`；2026-07-16 完成受限 Agent、原生 OpenCode adapter、静态 capability/permission probe、bounded evidence、完整进程树 timeout 和真实允许/拒绝 smoke。

**实现文档**：[R02 — OpenCode RTL Agent Protocol](tasks/R02-opencode-rtl-agent-protocol.md)

**验收证据**：Windows native OpenCode `1.18.2` / `opencode/deepseek-v4-flash-free`：静态 probe 通过并绑定 resolved config、resolved Agent permission、Agent、Skill 与 experiment digests；effective config 明确锁定 `autoupdate: false`、sharing/snapshot/formatter/LSP disabled、空 MCP/plugin/instructions 和 deny-only global permission。2026-07-17 R04 组合回归后的 resolved config digest 为 `sha256:d5109770f13d6e9db609fe66bd161efcbf1cfba29ad269ed46c13cc710fc8d03`，experiment digest 为 `sha256:5a98e3fdf4cedc9a4857c88bc22a2df3f9cdea4d8b1e007502ab1b92891cd310`，其余 permission/Agent/Skill digests 保持不变；18 项 adapter 单测和 2 项真实 smoke 通过。test-only Blank Generation 真实返回 `RTL_CHANGED`；独立 test-only permission probe 实际调用 write、收到 `status:error` 且目标文件不存在。确定性 fake native executable 覆盖固定 argv、无 fork/attach、prefix digest drift、隔离环境、成功、无变化、进程错误、timeout/tree kill、event/stderr 上限和 policy violation。真实 smoke 与临时合成输入均不计入评测数据；OpenCode 自有 session DB 仍会留存，shared evidence 不保存其宿主路径或 raw JSONL。
...
**状态**：`DONE`（2026-07-17）；R01/R02 行为保持兼容，结果仍为 non-authoritative compile-only。

**实现文档**：[R03 — Fixed Non-Authoritative Compile Adapter](tasks/R03-fixed-non-authoritative-compile-adapter.md)

**验收证据**：Windows x64 上通过 winget 锁定 `Icarus.Verilog 12.2022.06.11`，实际 executable 为 `C:\iverilog\bin\iverilog.exe`，规范化 identity 为 `Icarus Verilog version 12.0 (devel) (s20150603-1539-g2693dd32b)`；installer SHA-256 为 `a614057374dfaed5da0fe454cdeb410e54981fd85dbd28bd472f4ccb765deb84`，executable digest 为 `sha256:803b8844af2cc8ed70b5f08b07ffa749901280e81339617ca3e72cbbb852bd2b`，profile digest 为 `sha256:a3d0eff6e8da8396e3e68398badfa6bf50614dff4181f8667a75f5477fd930b1`。Core Loop 普通测试 10 files passed / 1 real-Agent-smoke file skipped、61 tests passed / 2 skipped；薄 CLI 3 tests passed；全仓 24 files passed / 1 skipped、165 tests passed / 2 skipped。独立真实 Icarus integration 1 file / 5 tests passed，覆盖 valid multi-file、syntax、missing top、blank source、elaboration、null target 与 deterministic rerun；CLI smoke 同时返回 `COMPILE_PASSED` 和 `COMPILE_ERROR`，且 run root 未出现 VVP output。确定性测试覆盖 include/preparation、manifest drift、version/missing/spawn/signal/unknown/timeout/unconfirmed termination、持续 drain、UTF-8 chunk、path redaction 和 status priority。Windows timeout 测试确认 bounded tree termination 与 `close`；非 Windows helper-tree 终止尚无真实证据，因此不声明 Linux readiness 或 formal Gate 能力。

**依赖**：R01。可与 R02 并行。

**目标**：用 repository-owned `iverilog-systemverilog-2012-null-v1` profile 对 mutable run workspace 执行预处理、语法解析和 top elaboration，不生成或运行 VVP，并返回结构化 feedback。

**交付物**：限定的 R01 schema 兼容修订、独立 `CompilePreparationResult`、不可变 profile mapping、版本 probe、受控 source/include preparation、stable manifest 检测、fixed argv/process adapter、持续 drain 的 timeout/output capture、diagnostic parser 和不可静默 skip 的真实 pass/error 集成测试。

**验收**：固定绝对 executable、`-g2012 -tnull -s <top>` argv、cwd、env 和 `shell: false`；Agent/fixture 不能覆盖 profile；include 在 spawn 前被拒绝；compile 前后 workspace 稳定且与 request digest 一致；持续 drain stdout/stderr 并以 `close` 完成 capture；`COMPILE_ERROR` 与 `TIMEOUT/TOOL_ERROR` 分离；结果不泄漏 host path并明确 non-authoritative/compile-only。R01 兼容修订不得改变 R02 已实现行为。
...
**ChipBench 接入进展**：固定 commit `74fe7d283225ae030ef59326a06111c9d372b48e`、archive SHA-256、683-file extracted manifest、223-case catalog、MIT reference 和 Provider source digest。生成集 45 题分类为 `BLANK_GENERATION`；8 个调试 split 共 178 题分类为 `PROMPTED_FUNCTIONAL_REPAIR`，不会伪装为 `SEEDED_COMPILE_REPAIR`。CLI 现按单一 split 生成 Kimi/OpenCode 或 Pi profile，并在候选编译通过后私有物化 reference/testbench、运行 Icarus/VVP、解析 mismatch；结果仍是非权威功能仿真证据。真实 prepare/check 已通过，当前实现验证结果见 Session Log。
