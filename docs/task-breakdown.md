...
**验收证据**：Windows native OpenCode `1.18.2` / `opencode/deepseek-v4-flash-free`：静态 probe 通过并绑定 resolved config、resolved Agent permission、Agent、Skill 与 experiment digests；test-only Blank Generation 真实返回 `RTL_CHANGED`；独立 test-only permission probe 实际调用 write、收到 `status:error` 且目标文件不存在。Core Loop 普通测试 6 files passed / 1 real-smoke file skipped、42 tests passed / 2 skipped；薄 CLI 2 tests passed；全仓 20 files passed / 1 skipped、145 tests passed / 2 skipped。确定性 fake native executable 覆盖固定 argv、prefix digest drift、隔离环境、成功、无变化、进程错误、timeout/tree kill、event/stderr 上限和 policy violation。真实 smoke 与临时合成输入均不计入评测数据；OpenCode 自有 session DB 仍会留存，shared evidence 不保存其宿主路径或 raw JSONL。
